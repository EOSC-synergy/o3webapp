...
describe("test EditModelGroupModal rendering", () => {
    ...
    it("renders correctly when open", () => {
        const { baseElement, queryByTestId } = render(<Provider store={store}><EditModelGroupModal isOpen={true} onClose={() => {}} modelGroupId={0}/></Provider>);
        expect(baseElement).toMatchSnapshot();
        expect(queryByTestId(/modal-wrapper/)).toBeInTheDocument();
    });

    it("renders correctly when close", () => {
        const { baseElement, queryByTestId } = render(<Provider store={store}><EditModelGroupModal isOpen={false} onClose={() => {}} modelGroupId={0}/></Provider>);
        expect(baseElement).toMatchSnapshot();
    ...
    it("closes the modal correctly when changes are discarded", () => {
        ...
        const { getByTestId } = render(<Provider store={store}><EditModelGroupModal isOpen={true} onClose={onClose} modelGroupId={0}/></Provider>);
...
describe("test the functionality of EditModelGroupModal", () => {
    ...
    it("correctly shows the intermediate and checked column header icon", () => {
        const { queryByTestId, getByTestId} = render(<Provider store={store}><EditModelGroupModal isOpen={true} onClose={() => {}} modelGroupId={0}/></Provider>);
