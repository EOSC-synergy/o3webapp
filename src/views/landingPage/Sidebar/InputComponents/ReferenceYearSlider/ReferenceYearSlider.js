...
import { setReferenceYear } from "../../../../../store/referenceSlice";
...
function ReferenceYearSlider(props) {
    ...
    const selectedYear = useSelector(state => state.reference.OCTS.year);

    /** Handles the change of the reference year slider if it's is modified.*/
    const handleChangeForRefYear = (event) => {
        dispatch(setReferenceYear({year: event.target.value}));
    };
