...
import { setReferenceModel } from "../../../../../store/referenceSlice";
...
function ReferenceModelSelector(props) {
    ...
    const selectedModel = useSelector(state => state.reference.OCTS.model);

    /** Handles the change of the reference model selection if it's is modified.*/
    const handleChangeForRefModel = (event) => {
        dispatch(setReferenceModel({model: event.target.value}));
    };
